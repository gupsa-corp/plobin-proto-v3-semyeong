<div>
    ...
    <div class="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <div class="text-2xl font-semibold text-gray-900">{{ $statistics['total'] ?? 0 }}</div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <div class="text-2xl font-semibold text-gray-900">{{ $statistics['active'] ?? 0 }}</div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <div class="text-2xl font-semibold text-gray-900">{{ $statistics['templatesCount'] ?? 0 }}</div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <div class="text-2xl font-semibold text-gray-900">{{ $statistics['errors'] ?? 0 }}</div>
                            ...
                            <button wire:click="sortBy('created_at')" class="group inline-flex">
                                ...
                                <span class="ml-2 flex-none rounded text-gray-400 group-hover:visible group-focus:visible">
                                    @if($sortBy === 'created_at')
                                        ...
                                        @else
                                            <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M3 3a1 1 0 000 2h11a1 1 0 100-2H3zM3 7a1 1 0 000 2h7a1 1 0 000-2H3zM3 11a1 1 0 100 2h4a1 1 0 100-2H3zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z"/>
                                            </svg>
                                        @endif
                                    @endif
                                </span>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($sandboxes as $sandbox)
                        <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $sandbox['name'] ?? 'Unknown' }}
                                            @if($sandbox['is_current'] ?? false)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if(($sandbox['status'] ?? 'active') === 'active')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $sandbox['size'] ?? 'Unknown' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ number_format($sandbox['file_count'] ?? 0) }}개
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $sandbox['created_at'] ?? 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    <button wire:click="selectSandbox('{{ $sandbox['name'] }}')" type="button"
                                    ...
                                    <button wire:click="confirmDelete('{{ $sandbox['name'] }}')" type="button"
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        정말로 샌드박스 <strong>'{{ $deleteTargetName }}'</strong>을(를) 삭제하시겠습니까? 
                                        이 작업은 되돌릴 수 없으며, 샌드박스의 모든 파일과 데이터가 영구적으로 삭제됩니다.
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('sandbox-selected', (event) => {
                if (typeof window.showNotification === 'function') {
                    window.showNotification('success', event[0].message);
                } else {
                    alert(event[0].message);
