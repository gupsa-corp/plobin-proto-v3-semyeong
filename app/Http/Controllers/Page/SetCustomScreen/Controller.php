...
class Controller extends \App\Http\Controllers\Controller
{
    public function __invoke($id, $projectId, $pageId, Request $request)
    {
        try {
            ...
            // 샌드박스가 설정되어 있는지 확인
            if (empty($page->sandbox_folder)) {
                return redirect()->back()->with('error', '커스텀 화면을 사용하려면 먼저 샌드박스를 선택해야 합니다.');
            }

            $sandboxCustomScreenFolder = $request->input('custom_screen', '');

            if (!empty($sandboxCustomScreenFolder)) {
                $page->update([
                    'sandbox_custom_screen_folder' => $sandboxCustomScreenFolder,
